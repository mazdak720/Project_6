...
def query_category(category):
    ...
    return {'category_name':category,'category_id':categoryid,'pages':[{'page_id':page['pageid'],'title':(unidecode(
...
def query_page(pageid):
    ...
    title = response.json()["query"]["pages"][pageid]["title"]
    
    cleantext = BeautifulSoup(html,'lxml').text
    cleantext = text_cleaner(cleantext)
    summary = get_summary(cleantext)
    
    return OrderedDict([('page_id',int(pageid)), ("title",title), ('summary',summary),('page',cleantext),('html',html)])
